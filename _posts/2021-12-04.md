...
# Podman
Podman is a daemonless, rootless, container engine. Podman is designed to be a drop in replacment for Docker. You can even run Docker Compose files with a little extra setup. Some other advantages of podman is the ability to generate systemd service files or kubernetes Pod yaml files. 
...
Like Docker, if you need to access the container from a browser, you need to tell podman what port the container will listen in on. The syntax is '-p host:container'. In the example below, we are going to access Jupyter Notebooks from port 6000. The container internal port is 8888.
...
```
With the above command we create a container named "jupyter", accessible from localhost:6000. Files will be saved and accessible from the host at ~/jupyter. Using Jupyter’s scipy-notebook image from Docker.io. Podman will check docker.io for new images. And notebook token is disabled.
...
If everything is working as intended. All that is left is to generating the systemd service file, enable it and see if it works.
...
Podman is great for developing and testing Kubernete Pods. Podman can converting docker or docker-compose file to kubernetes yaml. 
